...
func transcode(streamCtx *streamContext) {
	...
	case "CAM":
		...
		cmd = exec.Command("nice", "-n", "10", "ffmpeg", "-nostats", "-i", in, "-vsync", "2", "-c:v", "libx264", "-c:a", "aac", "-b:a", "128k", "-crf", "26", out)
	case "PRES":
		cmd = exec.Command("nice", "-n", "9", "ffmpeg", "-nostats", "-i", in, "-vsync", "2", "-c:v", "libx264", "-tune", "stillimage", "-c:a", "aac", "-b:a", "128k", "-crf", "20", out)
	case "COMB":
		cmd = exec.Command("nice", "-n" ,"8", "ffmpeg", "-nostats", "-i", in, "-vsync", "2", "-c:v", "libx264", "-c:a", "aac", "-b:a", "128k", "-crf", "24", out)
	default:
