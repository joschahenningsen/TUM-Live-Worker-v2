// Package main provides a simple edge server that proxies requests for TUM-Live-Worker and caches immutable files.
package main
...
var (
	...
	allowedRe = regexp.MustCompile("^/[a-zA-Z0-9]+/([a-zA-Z0-9]+/)*[a-zA-Z0-9]+\\.(ts|m3u8)$") // e.g. /vm123/live/stream/1234.ts
...
func main() {
	log.Println("Starting edge server version " + VersionTag)
	port := os.Getenv("PORT")
	if port == "" {
		port = ":8089"
	}
...
// fetchFile fetches a file from the origin server and persists it in the cache directory.
